...
Serializable {
    ...
    private void writeObject(ObjectOutputStream stream) throws IOException {
        ...
        for (int i2 = 0; i2 < outlinePaintCount; ++i2) {
            SerialUtilities.writePaint(this.outlinePaintSequence[i2], stream);
        }
        int strokeCount = this.strokeSequence.length;
        stream.writeInt(strokeCount);
        for (int i3 = 0; i3 < strokeCount; ++i3) {
            SerialUtilities.writeStroke(this.strokeSequence[i3], stream);
        }
        int outlineStrokeCount = this.outlineStrokeSequence.length;
        stream.writeInt(outlineStrokeCount);
        for (int i4 = 0; i4 < outlineStrokeCount; ++i4) {
            SerialUtilities.writeStroke(this.outlineStrokeSequence[i4], stream);
        }
        int shapeCount = this.shapeSequence.length;
        stream.writeInt(shapeCount);
        for (int i5 = 0; i5 < shapeCount; ++i5) {
            SerialUtilities.writeShape(this.shapeSequence[i5], stream);
        }
    }
    ...
    private void readObject(ObjectInputStream stream) throws IOException, ClassNotFoundException {
        ...
        for (int i2 = 0; i2 < outlinePaintCount; ++i2) {
            this.outlinePaintSequence[i2] = SerialUtilities.readPaint(stream);
        }
        int strokeCount = stream.readInt();
        this.strokeSequence = new Stroke[strokeCount];
        for (int i3 = 0; i3 < strokeCount; ++i3) {
            this.strokeSequence[i3] = SerialUtilities.readStroke(stream);
        }
        int outlineStrokeCount = stream.readInt();
        this.outlineStrokeSequence = new Stroke[outlineStrokeCount];
        for (int i4 = 0; i4 < outlineStrokeCount; ++i4) {
            this.outlineStrokeSequence[i4] = SerialUtilities.readStroke(stream);
        }
        int shapeCount = stream.readInt();
        this.shapeSequence = new Shape[shapeCount];
        for (int i5 = 0; i5 < shapeCount; ++i5) {
            this.shapeSequence[i5] = SerialUtilities.readShape(stream);
        }
    }
