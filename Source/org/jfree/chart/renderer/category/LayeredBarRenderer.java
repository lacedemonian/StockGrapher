...
implements Serializable {
    ...
    protected void calculateBarWidth(CategoryPlot plot, Rectangle2D dataArea, int rendererIndex, CategoryItemRendererState state) {
        ...
        if (dataset != null) {
            ...
            if (rows * columns > 0) {
                state.setBarWidth(used / (double)dataset.getColumnCount());
            } else {
    ...
    protected void drawHorizontalItem(Graphics2D g2, CategoryItemRendererState state, Rectangle2D dataArea, CategoryPlot plot, CategoryAxis domainAxis, ValueAxis rangeAxis, CategoryDataset data, int row, int column) {
        ...
        if (state.getInfo() != null && (entities = state.getEntityCollection()) != null) {
            ...
            CategoryItemEntity entity = new CategoryItemEntity(bar, tip, url, data, row, data.getColumnKey(column), column);
            entities.add(entity);
        }
    }
